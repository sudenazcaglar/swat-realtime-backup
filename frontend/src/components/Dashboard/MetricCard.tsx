...
export const MetricCard: React.FC<MetricCardProps> = ({ sensor }) => {
  const getTrendIcon = () => {
    // TrendPoint[] -> number[]
    const recentValues = sensor.trend.slice(-5).map(p => p.value);
    const current = sensor.value;

    if (recentValues.length === 0) return <Minus className="w-4 h-4 text-gray-400" />;
  ...
  const getStatusColor = () => {
    switch (sensor.status) {
      ...
      default:
        return 'border-gray-600 bg-gray-800/50 hover:border-cyan-500/50';
    }
  };
  ...
  // TrendPoint[] -> number[]
  const sparklinePoints = sensor.trend.slice(-15);
  const sparklineData = sparklinePoints.map(p => p.value);
  const sparklinePath = createSparklinePath(sparklineData);

  const sparkMin = sparklineData.length ? Math.min(...sparklineData) : sensor.value;
  const sparkMax = sparklineData.length ? Math.max(...sparklineData) : sensor.value;
  ...
  return (
    <div className={`rounded-xl border-2 p-6 transition-all duration-300 hover:scale-105 ${getStatusColor()}`}>
      ...
      <div className="flex items-baseline space-x-2 mb-4">
        <span className="text-3xl font-bold text-white">{formatValue(sensor.value)}</span>
      ...
      <div className="relative h-16 mb-2">
        <svg width="120" height="60" className="w-full h-full">
          ...
          {sparklineData.map((value, index) => {
            const x = sparklineData.length > 1 ? (index / (sparklineData.length - 1)) * 120 : 0;
            const y = 60 - ((value - sparkMin) / sparkRange) * 60;
          ...
          <circle
            cx="120"
            cy={60 - ((sensor.value - sparkMin) / sparkRange) * 60}
            r="3"
            fill="#06B6D4"
            stroke="#1F2937"
            strokeWidth="1"
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-gray-400">{sparklineData.length} samples</span>
      </div>
    </div>
  );
};
